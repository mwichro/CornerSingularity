...
singularity: the tangent stays strongly elliptic and Fredholm, the standard Kondratiev edge exponents survive, and the
...
the cubic coefficient is the more complete of the two: the consistent graded-wedge solve of \S\ref{sub:numb} returns
$b<0$ \emph{throughout} the physical $(\nu,\hat K)$ rectangle --- the normalisation-invariant feedback ratio stays below
its critical value $-1$ everywhere, with the smallest margin near the incompressible corner --- so subcriticality
(Assumption~\ref{ass:subcrit}) is numerically established, not merely posited. What remains genuinely beyond reach here is
