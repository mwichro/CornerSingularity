...
\begin{lemma}[Localized surface instability $\Rightarrow$ structural instability]
    ...
    \end{equation}
    Consequently, with $\mu_1(0)>0$ from Lemma~\ref{lem:tc}(i) and granted the pre-collapse floor (FP)(i) with
    $t_*<t_{\mathrm{coll}}$ (hypothesis (iii) of Lemma~\ref{lem:tc}), there is a first critical load $t_c\in(0,t_*]$
    with $\mu_1(t_c)=0$, and the ordering $t_c<t_{\mathrm{coll}}$ follows.
\end{lemma}
...
\begin{proof}
    ...
    threshold \cite{Biot1963,Hong2009}:\claude{These references treat the incompressible/gel half-space; existence,
    exponential decay, and $q(\lambda_\parallel)<0$ for the \emph{compressible} $f_{\log}$ material are used here but
    not covered by them. The compressible secular condition and its root are exactly what the surface-instability
    computation of \S\ref{sub:numbiot} implements --- state that secular analysis in the appendix (or cite a
    compressible reference) so Step~1 rests on something in the paper.} for each surface wavevector
    $\bm{k}\in\mathbb{R}^{d-1}$, $k:=|\bm{k}|$, there is a
    surface mode
    \begin{equation}
        \label{eq:biotmode}
...
\begin{remark}[Reading, and the computational route it recommends]
    ...
    small-$\hat K$ regime only; on the finite $(\nu,\hat K)$ rectangle the criterion $\Gamma>1$ must be checked, and
    \S\ref{sub:numb} now reports that check: certified at every tested rectangle point on the surrogate cell, while
    hypothesis (b) itself remains untestable there because the monotone grading removes the very resonance the
    proposition exploits.
...
\begin{remark}[What the identities change]
    ...
    leave its evaluation to the planned wedge computation.\claude{This envelope-extremum question needs no wedge
    solver: the Stroh exponents and amplitude ratios are already computed by the surface-instability solve of
    \S\ref{sub:numbiot}, so the extremum-vs-tube comparison is a closed-form evaluation on existing data. Given that
    the accessible evidence currently \emph{leans against} \eqref{eq:dphisign} (deep-skin ratio ${\approx}0.6$,
    drifting down), answering it should precede any further reliance on the relief floor.}
\end{remark}
...
% (BC) is now derived, not independent: record its relation to the standing path assumption and the rewired lem:tc.
\noindent Under the standing path assumption (FP)(i) of \S\ref{sec:iba}, (BC) is no longer an independent postulate:
once the destabilisation load satisfies $t_*<t_{\mathrm{coll}}$, Lemma~\ref{lem:tc} \emph{concludes}
$t_c\le t_*<t_{\mathrm{coll}}$ and \eqref{eq:bc} holds with $c=c(t_*)$. We keep the name (BC) for its mnemonic value;
its entire quantitative content is the single lower bound $t_{\mathrm{coll}}>t_*$, which
Proposition~\ref{prop:tipdisc} confines to the tip disc.
...
\begin{lemma}[Attainment under (BC)]
    ...
    the compact cylinder $[0,t_c]\times\overline{B_\rho}$ --- a standing regularity of the load path in the spirit of
    the selection continuity of Assumption~\ref{ass:fp}, automatic for a
...
\begin{enumerate}
    ...
    \item \emph{Subcritical sign pattern}, $b<0<d$ (Assumption~\ref{ass:subcrit}): the directions in which the cubic and
          ...
          Proposition~\ref{prop:certdeg}, and on the finite rectangle certified by the inversion-free criterion of
          Corollary~\ref{cor:cert}, evaluated in \S\ref{sub:numb} (Remarks~\ref{rem:bsign}, \ref{rem:certnum}); the
          quintic, by contrast, is \emph{computed negative} through the sixth-order reduction
          (Lemma~\ref{lem:quintic}, \S\ref{sub:numb}), so the classical saturation clause fails and the arrested
          post-snap state is carried by (ND) alone (Remark~\ref{rem:quinticstatus}). The
...
\begin{table}[t]
    ...
    \begin{tabular}{lll}
        ...
        Subcritical cubic $b<0$ (Ass.~\ref{ass:subcrit})                          & assumed                                                        & sign \textbf{proved} $b_\psi{\le}0$ (Lem.~\ref{lem:fbsign}), magnitude \emph{certified numerically} (Cor.~\ref{cor:cert}, \S\ref{sub:numb}); quintic \textbf{computed} $d{<}0$: no polynomial saturation, arrest via (ND) (Lem.~\ref{lem:quintic})     \\
        \bottomrule
    \end{tabular}
