...
\begin{proposition}[Blow-up reduction to the model wedge]
    ...
    \begin{enumerate}
        ...
        \item[\textup{(ii)}] \emph{Operator convergence away from the tip.} On every annulus
              ...
              $\mathcal{L}_W$ obtained by freezing along the self-similar field $\hat{\bm{u}}_W$. Consequently the leading
              singular exponent and angular profile of $\widehat{\mathcal{L}}_\lambda$ agree with those of $\mathcal{L}_W$
              in the limit $\lambda\to0$, and only the dimensionless amplitude $K$ (equivalently the normalised
              stress-intensity $\hat K$ of Remark~\ref{rem:sifparam}) survives from the global problem. We emphasise what
              is \emph{not} asserted: this annular ($C^0$, away-from-tip) convergence does not by itself control the
              operator in a weighted norm \emph{up to} $r=0$, and the resulting agreement of the full Lyapunov--Schmidt
              reduced data is here a leading-order, formal statement rather than a quantitative $\mathcal{O}(\lambda^{g})$
              spectral-perturbation theorem; the tip enters through the weight, and a uniform-to-the-tip operator-norm
              estimate would require the inner vertex theory left open in Remark~\ref{rem:core}.
...
\begin{proof}
    ...
    $\{r_0\le r\le R\}$ bounded away from $r=0$. The leading exponent and angular profile, being determined by the
    coefficient values on such annuli together with the (fixed) Dirichlet--Neumann boundary conditions, therefore
    converge to those of $\mathcal{L}_W$. We do \emph{not} claim a weighted operator-norm estimate uniform up to $r=0$:
...
\begin{proof}
    ...
    c^{-1}$. The matching \emph{upper} bound is not part of (BC) --- which constrains only $\lambda_{\min}$ --- but is
    automatic: the fundamental state $(t,\bm x)\mapsto\bm F_0(t,\bm x)$ is continuous on the compact set
    $[0,t_c]\times\overline{B_\rho}$ (it solves the equilibrium problem along the load path with bounded data), so all
...
with no quadratic term by symmetry, and the
kinematic relief that regularises the tangent holds in every dimension once the asymptotic-rotation hypothesis is
granted. Notably, the spectral postulates of the abstract theory are sharply reduced: attainment is \emph{proved} under
(BC), and simplicity is reduced to genericity within the symmetry sector carrying the critical mode rather than posited
outright; the first-order volume preservation (rotation condition) that the relief once posited as an exact identity is
