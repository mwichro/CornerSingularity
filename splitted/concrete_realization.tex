...
\begin{remark}[The flat reduction is degenerate; the corner is what makes $b$ exist]
    ...
    well-defined number. Thus the corner is not merely the setting in which $b$ happens to be computed: it is what makes
    the reduced cubic \emph{exist} at all. This sharpens Proposition~\ref{prop:bcount} --- $b$ cannot be a bare-wedge,
    $\hat K$-independent constant, because the $\hat K\to0$ (flat) limit is not a regular point of the reduction but the
    degenerate one.
\end{remark}
...
\begin{lemma}[\textup{(BC)} is a model-wedge inequality]
    \label{lem:bcwedge}
    The buckling side of (BC), $t_c\le t_*$, reduces to a \emph{scale-invariant} threshold of the model wedge
    $\mathcal{L}_W$ of Proposition~\ref{prop:blowup}, depending on the geometry only through $\nu$ and free of the
    stress-intensity amplitude $\hat K$. \emph{Provided} the barrier-regularised collapse load $t_{\mathrm{coll}}$
    rescales as a wedge quantity --- the lower bound on $t_{\mathrm{coll}}$ left open in Remark~\ref{rem:bc} --- the full
    ordering $t_c<t_{\mathrm{coll}}$ is likewise amplitude-free and a property of $\nu$ alone.
\end{lemma}
...
\begin{proof}
    ...
    $\mathcal{L}_W$; this establishes the buckling side unconditionally. For the collapse load $t_{\mathrm{coll}}$ ---
    the value at which the barrier-regularised continuation of the self-similar field first reaches $\lambda_{\min}=0$
    --- the data $\lambda_{\min}(\hat{\bm{u}}_W)$ and the barrier $f_{\log}$ are homogeneous on $W$, which makes a
    wedge-scaling of $t_{\mathrm{coll}}$ \emph{plausible}; but the nonlinear threshold is set at the tip, where the
    linear prediction wants to collapse for any load, and a quantitative lower bound there is the content left open in
    Remark~\ref{rem:bc}. Granting that scaling, multiplying the load by a constant translates both thresholds equally,
    so the \emph{ordering} $t_c<t_{\mathrm{coll}}$ is amplitude-free: invariant under $\hat K\mapsto c\,\hat K$ and a
    property of $\nu$ and the $90^\circ$ wedge alone. By Proposition~\ref{prop:blowup}(ii) the finite-body corrections
    are $\mathcal{O}(\lambda^{g})$. \qed
\end{proof}
