...
$J=J_0+w\,D_1+w^2 d_\phi+\mathcal{O}(w^3)\ge\tfrac12 J_0+\tfrac12 w^2 d_\phi$ in $\mathbb{R}^d$ with $d_\phi$ the second
mixed discriminant of $\bm{F}_0$ and $\Grad\bm{\phi}$ (the second-order coefficient of the determinant,
$=\det\Grad\bm{\phi}$ for $d=2$), and the first-order term $D_1=\operatorname{cof}\bm{F}_0:\Grad\bm{\phi}$ is suppressed
--- not to zero, but to $\mathcal{O}(J_0/\sqrt{|\log J_0|})$, fast enough to be dominated by the second-order relief ---
by the barrier in every dimension via Lemma~\ref{lem:rot} (its proof uses only $f''(J)(\delta J)^2\le C$, valid for all
$d$). For the concrete model the only
...
abstract theory --- attainment and simplicity --- are no longer assumed but proved; what remains are two \emph{concrete}
