...
\begin{remark}[What (BC) costs, and why it is the right residue]
    ...
    thereby reduced to a pointwise floor on that single shrinking disc.
\end{remark}
...
\begin{lemma}[The barrier repels collapse in measure]
    ...
    \end{equation}
    Suppose the loading is displacement-controlled, so that $\bm u_0(t)$ minimises $\mathcal E_t=\int\Psi$ among
    admissible fields, and let $E(t):=\mathcal E_t(\bm u_{\mathrm{trial}})$ for any fixed admissible competitor
    (finite and explicit for the bodies (H), (C), e.g.\ the smooth ramp extension of the boundary data). Then for
    every $\delta\in(0,1)$,
    \begin{equation}
        \label{eq:measrep}
...
\begin{proof}
    ...
    \begin{equation*}
        ...
        \ \ge\ \lambda_e\log^2 J,
    \end{equation*}
    since $s-1-\log s\ge0$ at $s=\lambda_i^2$; this is \eqref{eq:psicoerc}. By minimality
...
\begin{proposition}[Collapse, if anywhere, is confined to the tip disc]
    \label{prop:tipdisc}
    Along the fundamental path, for $t<t_{\mathrm{coll}}$, let the Kondratiev expansion of
    Proposition~\ref{prop:blowup}(i) hold with the pointwise gradient bounds
    $\big\|\Grad\big(K r^\alpha\bm\Phi\big)\big\|\le c_\Phi\,|\hat K(t)|\,\mu_e^{-1}\ldots$ --- concretely, set
    $c_\Phi:=\max_\theta\big\|\alpha\,\bm\Phi\otimes\bm e_r+\bm\Phi'\otimes\bm e_\theta\big\|$ and let $C_\sharp(t)$
    bound the remainder, $|\Grad\bm u_0^\sharp|\le C_\sharp(t)\,(r/\rho)^{\alpha'-1}$
    \cite{KMR2001,MazyaRossmann2010}, with $M(t):=c_\Phi\,|\hat K(t)|\,+C_\sharp(t)$ bounded along the path. Then
    \begin{equation}
        \label{eq:tipfloor}
...
\begin{proof}
    For $t<t_{\mathrm{coll}}$ the state is uniformly non-degenerate on $\overline{B_\rho}$, the coefficients are
    bounded (Lemma~\ref{lem:attain}), and the linearised corner theory applies, so the expansion
    $\bm u_0=K r^\alpha\bm\Phi(\theta)+\bm u_0^\sharp$ holds with the stated bounds. Then
