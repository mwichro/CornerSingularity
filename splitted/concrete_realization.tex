...
\begin{lemma}[Neo-Hookean reduced coefficients are volumetric; the quadratic vanishes]
    ...
    \begin{enumerate}
        ...
        \item[\textup{(ii)}] \emph{The frozen cubic and quintic are sign-definite, and adverse to subcriticality.} With
              ...
              hypothesis on $d_\phi$. The quintic part carries $d_\phi^3$; under the orientation condition $d_\phi>0$
              (the generic, relief-orienting sign of the second mixed discriminant, \eqref{eq:dphisign} below) and
              $f'''_{\log}<0$ one gets $d_{\mathrm{dir}}<0$ --- the \emph{opposite} of the subcritical pattern $b<0<d$.
    \end{enumerate}
\end{lemma}
\begin{proof}
...
two-dimensional accident. The relief floor requires the \emph{orientation} of this second-order coefficient,
\begin{equation}
    \label{eq:dphisign}
    d_\phi>0 \qquad\text{(a.e.\ on $B_\rho$, the relief-orienting sign of the mode),}
\end{equation}
which we record as a genericity property of the critical mode, \emph{not} as a consequence of
Lemma~\ref{lem:sval} (that lemma concerns the transversal vanishing of $\lambda_{\min}$, a different field). Two
remarks make \eqref{eq:dphisign} the natural generic condition. First, $d_\phi=\det\Grad\bm\phi$ ($d=2$) is an analytic
function of the mode, so the locus $\{d_\phi=0\}$ is non-generic (measure zero among modes) and $d_\phi$ has a definite
sign on each nodal cell; second, $d_\phi>0$ is exactly the statement that the mode \emph{relieves} rather than deepens
the local volume change at second order --- the sign for which $\tfrac12 w^2 d_\phi$ in \eqref{eq:reliefd} is a floor
rather than a further compression. A mode with $d_\phi<0$ on a set of positive measure is collapse-\emph{driving} there
and is excluded by this condition; on the computed critical mode of \S\ref{sec:numerical} the relief orientation is the
observed one. The remaining points for the concrete model are that the collapse locus $\Sigma_{\mathrm{c}}$ is
generically the
...
dichotomy (Remark~\ref{rem:sifparam}): condition (BC) compares two \emph{scale-invariant stretch thresholds}, which are
one-parameter ($\nu$) wedge data on $\mathcal{L}_W$ (the conversion to a load ordering using only the monotone
load-to-stretch map, Lemma~\ref{lem:bcwedge}), whereas the cubic sign is an
