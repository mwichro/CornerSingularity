...
grading is what regularises the coefficient. The linear ingredients are in hand and cross-validated
(\S\ref{sub:numpencil}--\S\ref{sub:numbiot}); the graded second-order solve is work in progress.
%
% TODO (graded-substrate / corner b-solver, the open computation of this subsection):
%  * Build the variable-coefficient (X-graded) buckling + Lyapunov-Schmidt solver in a single CONSISTENT weak
%    (Galerkin) formulation -- take the critical mode phi from the Galerkin eigenproblem, NOT a strong-form collocation
%    null vector (mixing the two pollutes the second-order source; see handoff.md).
%  * Represent the second harmonic in real (cos/sin) DOFs to avoid the complex-conjugation ambiguity in the load.
%  * The grading (parameter Khat) lifts the all-wavenumber Biot degeneracy (Rem.~\ref{rem:flatdegenerate}); confirm the
%    selected mode is discrete and Q at the 2nd harmonic is non-resonant before trusting b.
%  * Output: sign of b(nu,Khat) over the physical rectangle -> a phase-diagram figure (cf. Fig.~\ref{fig:williams}).
%  * Validated linear building blocks: williams_clampedfree.py / mellin_chebyshev_pencil.py (alpha) and
%    surface_instability.py / creasing_buckling.py Stage 1 (lambda_star). creasing_buckling.py Stage 2 is WIP/unconverged.
