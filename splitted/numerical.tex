...
\begin{center}
    ...
    \end{tabular}
\end{center}
This is the input that controls the \emph{buckling side} of (BC): the buckling load $t_c\le t_*$ is attained while the
fundamental stretch is still $\lambda_\parallel=\lambda_\star=\mathcal{O}(1)$, away from collapse. It settles the
ordering $t_c<t_{\mathrm{coll}}$ only together with a lower bound on the competing collapse load $t_{\mathrm{coll}}$,
which the barrier keeps high but whose quantitative tip bound is left open (Lemma~\ref{lem:bcwedge},
Remark~\ref{rem:bc}). The computation is reproduced by
