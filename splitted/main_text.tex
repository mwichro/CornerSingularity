...
\begin{theorem}[Well-posedness of the linearised step, tensile corner]
    ...
    Assumptions~\ref{ass:reg}--\ref{ass:mild}; the pure logarithmic barrier $f(J)=-\mu_e\log J$, for which
    $f''(J)J^2=\mu_e>0$ globally and $\Theta\equiv\mu_e$, is the model exemplar).} Then the dilation core is empty,
...
\begin{remark}[The dilation core]
    ...
    must reorganise, changing the ansatz rather than the tangent. The dilation core is thus a feature of the specific
    large-$J$ tail of $f_{\log}$ interacting with the assumed kinematics, not a physical catastrophe.
\end{remark}
