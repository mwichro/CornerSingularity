...
\end{equation}
For the barrier penalty $f_{\log}$, $\Theta(J)=\mathcal{O}(\log\tfrac1J)$ as $J\to0$ (cf. \eqref{eq:Thetacompress}) is
subordinate to $\|\bm{F}^{-1}\|^2$; substituting $\lambda_{\min}\sim\epsilon^{2/3}$ and $J\sim\lambda_{\min}$,
\begin{equation}
    \max_{\bm{x} \in \Omega} \| \mathbb{L}(\bm{x}, \epsilon) \| \leq
    C\,\Theta(J)\,\|\bm{F}^{-1}\|^2 \sim
    \mathcal{O}\!\big(\epsilon^{-4/3}\,\log\tfrac1\epsilon\big) .
\end{equation}
The imperfection thus converts the unbounded tangent of the perfect compressive path into a finite quantity that
diverges only algebraically (up to a logarithm) as the noise is removed, at the rate $\epsilon^{-4/3}\log\tfrac1\epsilon$.
The logarithmic factor is the same amplification identified in \eqref{eq:Lcompress}; it is the compressive counterpart
of the benign $\log\tfrac1r$ growth in tension, and the analytic trace of the energy barrier that makes this whole
bifurcation analysis well posed.
