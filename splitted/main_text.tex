...
polar coordinates $(r,\theta)$ by $r\in[0,R]$, $\theta\in[0,\pi/2]$, and write $B_\rho := \{\bm{x}\in\Omega :
    |\bm{x}|<\rho\}$ for the corner neighbourhood.
\begin{itemize}
...
$0<\lambda_{\min}(\bm{F})\le\lambda_{\max}(\bm{F})$ are the singular values of $\bm{F}$ (the principal stretches), and
we recall $\|\bm{F}^{-1}\| = \lambda_{\min}(\bm{F})^{-1}$.
...
characteristic of a Dirichlet--Neumann opening, where $J\sim r^{\alpha-1}$.) The tangent norm near the corner then
obeys
\begin{equation}
...
\begin{proof}
    ...
    \]
    where the Korn inequality \eqref{eq:korn} converts ellipticity of the symmetric part into control of the full gradient.
    ...
    $k$, and we obtain \eqref{eq:Agaarding} with $\gamma_1=\tfrac12\gamma_0 c_K^{-1}$.
...
off zero (Lemma~\ref{lem:relief}, with the volume-preserving character of the mode \emph{forced} by the energy barrier,
...
appears in both. Two practical consequences follow. First, the bifurcation is the corner-localised analogue of the
classical \emph{creasing} instability of compressed elastomers \cite{Biot1963,Hong2009}, now seated at a geometric
Dirichlet--Neumann junction rather than on a flat surface. Second, the dichotomy is a concrete warning for computation:
a solver that continues through the instability will see the conditioning degrade as $\epsilon^{-4/3}$ and should
either admit the snap or refine inside the tube \eqref{eq:locallength}; the load-control reduction \eqref{eq:koiter}
tells it when the snap is due. The reduction to scalar imperfect-bifurcation form follows the classical theory
