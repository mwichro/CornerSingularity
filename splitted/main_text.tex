...
\begin{theorem}[Well-posedness of the linearised step, tensile corner]
    ...
    \emph{(A) Globally convex-volumetric barrier ($f''\ge0$, e.g.\ $f_{\mathrm{quad}}$, or $f_{\log}$ capped at
    $J_{\max}\le J_*$).} Then the dilation core is empty, $r_*=0$, ellipticity holds on the \emph{full} corner
...
\begin{proof}
    ...
    comparable there; instead we read the bound as identifying the $\mathcal{O}(\log\tfrac1r)$ coefficient growth as a
    perturbation subordinate to the principal (constant-coefficient elasticity) symbol with relative bound $0$. By the
    Kondratiev--Maz'ya stability of the weighted estimate under such subordinate perturbations \cite{KMR2001}, the
    admissible weight interval is unmoved and \eqref{eq:Abounded} holds on $V^{1,2}_{\beta_\star}(B_\rho)$. In both
    cases the logarithmic growth does not shift the admissible interval; no singular weight is ``absorbed'' into
    $\bm{v}$.
...
\begin{proposition}[Stable physical branch: bounded tangent]
    ...
    compressed branch $\lambda_{\min}\ge\lambda_c(t)\ge\lambda_c(t_c)>0$. After the snap the configuration is a
    \emph{finite-energy} equilibrium of the barrier model: since $f_{\log}(J)\to\infty$ as $J\to0^+$, a uniform energy
    bound forces $J=\det\bm{F}\ge J_{\mathrm{floor}}>0$ pointwise, with $J_{\mathrm{floor}}$ depending only on the energy
    level, hence (the remaining $d-1$ stretches being $\mathcal{O}(1)$ on the crease) $\lambda_{\min}=\mathcal{O}(1)$
    uniformly in $\epsilon$. This is the finite-amplitude counterpart of the small-$w$ relief mechanism of
    Lemma~\ref{lem:relief}, which it does not invoke (the lemma is local, $|w|\le w_0$); the barrier alone furnishes the
    bound at $\mathcal{O}(1)$ amplitude. Hence on the physical branch
    \begin{equation}
