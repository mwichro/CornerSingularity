...
\begin{lemma}[Determinant relief]
    ...
    Lemma~\ref{lem:rot} below, exactly rather than assumed). Let
    $d_\phi:=\tfrac12\,\tfrac{d^2}{dw^2}\big|_{0}\det(\bm{F}_0+w\,\Grad\bm{\phi})>0$ be the second-order coefficient of
    the determinant --- the second mixed discriminant of $\bm{F}_0$ ($d-2$ copies) with $\Grad\bm{\phi}$ (two copies),
    which for $d=2$ reduces to $\det(\Grad\bm{\phi})$. Then
    \begin{equation}
        \label{eq:Jexp}
        J = \det\bm{F} = J_0 + w^2\,d_\phi + \mathcal{O}(w^3),
    \end{equation}
    so that, with $\lambda_c=\lambda_{\min}(\bm{F}_0)$ and the remaining $d-1$ stretches of $\bm{F}$ bounded,
...
\begin{proof}
    ...
    $D_2=d_\phi$. The first-order term $D_1$ vanishes by \eqref{eq:rotcond}, leaving \eqref{eq:Jexp} (the
    $\mathcal{O}(w^3)$ collecting $D_3,\dots,D_d$). Since $J=\prod_i\lambda_i$ and the $d-1$ largest stretches are
