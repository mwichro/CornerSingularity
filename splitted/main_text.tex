...
We combine \eqref{eq:Hessbound} with (NC) ($\|\bm{F}^{-1}\|^2\le c^{-2}$) and the tensile kinematics. Two kinematic
regimes are possible at fixed angle: either exactly one stretch diverges, $\lambda_{\max}\sim|K|\,r^{\alpha-1}$ with the
remaining stretches bounded, giving $J=\det\bm{F}\sim r^{\alpha-1}$; or two stretches diverge and the corresponding
subdeterminant gives $J\sim r^{2(\alpha-1)}$. For the norm bounds of this subsection the distinction is immaterial ---
under Assumption~\ref{ass:mild} both give $\Theta(J)=\mathcal{O}\big((\log\tfrac1r)^p\big)$ --- and we quote the
one-large-stretch scaling $J\sim r^{\alpha-1}$ as the representative case; the angular structure of the two regimes
under the Williams ansatz, which turns out to \emph{sharpen} the corner theory, is resolved in
Lemma~\ref{lem:screen} below. The tangent norm near the corner then obeys
\begin{equation}
...
\begin{center}
    \begin{tabular}{lll}
        ...
        tempered log \eqref{eq:ftempered} & $\sim |\log J|$ \ (softest) & $\mathcal{O}(1)$ \\
...
% Who satisfies mild growth *and* global convexity: the class is characterised and populated here, since neither
% standard penalty does, and the benign corner case of Theorem A would otherwise be a theorem about an empty class.
The benign corner case of Theorem~\ref{thm:A} below will require Assumption~\ref{ass:mild} \emph{together with} global
convexity $f''\ge0$, and neither standard penalty satisfies both: $f_{\mathrm{quad}}$ fails mild growth, $f_{\log}$
fails convexity past $J_*$ (Remark~\ref{rem:SEcaveat}). Whether the combined class is nonempty --- with both elastic
moduli still free --- deserves an explicit answer, and it has a clean one.

\begin{remark}[The convex-mild class: forced sign, and an explicit two-parameter exemplar]
    \label{rem:tempered}
    Write $t=\log J$ and $h(t):=f'(J)J$, so that $f''(J)J^2=h'(t)-h(t)$; convexity reads $h'\ge h$ and
    \eqref{eq:Thetagen} becomes $\Theta=|h'-h|+|h|$.
    \emph{(i) Forced sign.} If $f''\ge0$ and $\Theta(J)=\mathcal{O}((\log J)^p)$, then $f'(J)\le 0$ for \emph{all}
    $J$: were $f'(J_0)>0$, convexity would give $f'(J)\ge f'(J_0)$ for $J\ge J_0$, hence
    $\Theta(J)\ge f'(J)J\ge f'(J_0)J$, algebraic growth. Every admissible material is therefore
    \emph{never volumetrically tensile}: the bulk stress resists compression but never pushes back against dilation.
    This is exactly where $f_{\log}$ exits the class --- its volumetric stress $h(t)=-\mu_e+2\lambda_e t$ turns
    \emph{positive} past $J_p=e^{\mu_e/(2\lambda_e)}$, and mild growth then forbids the convexity needed to sustain it.
    \emph{(ii) The class is nonempty, with both moduli free.} Saturate the linear $h$ of $f_{\log}$: for a temper scale
    $0<T<\mu_e/(2\lambda_e)$ set $h_T(t):=-\mu_e+2\lambda_e T\tanh(t/T)$, i.e.
    \begin{equation}
        \label{eq:ftempered}
        f_T(J) \;=\; -\mu_e\log J \;+\; 2\lambda_e T^2\,\log\cosh\!\Big(\frac{\log J}{T}\Big) .
    \end{equation}
    Then $f_T\in C^\infty(0,\infty)$, $f_T(1)=0$, $f_T'(1)=-\mu_e$, $f_T''(1)=\mu_e+2\lambda_e$ --- the linearised
    moduli, hence the Poisson ratio, are \emph{identical} to those of $f_{\log}$, and indeed
    $f_T=f_{\log}+\mathcal{O}(\log^4 J)$ near $J=1$ --- while globally
    \begin{equation*}
        f_T''(J)J^2 \;=\; \mu_e-2\lambda_e T\tanh\big(\tfrac{t}{T}\big)+2\lambda_e\operatorname{sech}^2\big(\tfrac{t}{T}\big)
        \;\ge\; \mu_e-2\lambda_e T \;>\;0,
        \qquad
        \Theta_T(J)\;\le\;3\mu_e+2\lambda_e \quad (p=0),
    \end{equation*}
    with the barrier $f_T(J)\sim(\mu_e+2\lambda_e T)\,|\log J|\to\infty$ as $J\to0^+$, and $h_T<0$ everywhere
    (the forced sign realised). The family interpolates between the pure logarithmic barrier ($T\to0$) and $f_{\log}$
    itself (the inadmissible limit $T\to\infty$); its energy grows like $|\log J|$ under dilation, \emph{softer} than
    $f_{\log}$'s $\log^2 J$, which is the analytic mechanism by which it keeps both convexity and mild growth.
    \emph{(iii) One-sided temper.} Since $f_T$ and $f_{\log}$ agree to fourth order in $\log J$ at $J=1$, one may glue
    $f_{\log}$ on $J\le1$ to $f_T$ on $J\ge1$ with $C^3$ regularity: the resulting penalty is \emph{pointwise
    identical} to $f_{\log}$ throughout compression --- every compressive statement in this paper is untouched ---
    while the dilation tail is tempered, $f''>0$ holds globally, and the dilation core of Lemma~\ref{lem:Aellip}
    disappears ($r_*=0$). The case split of Theorem~\ref{thm:A} is thus a property of the untempered $\log^2$ tail of
    $f_{\log}$, in the large-$J$ regime that (as \S\ref{sub:growth} already observed) was never physically
    constrained, and it can be removed at no cost to the compressive analysis by this substitution.
\end{remark}
...
\begin{proof}
    ...
    and $\det\bm{F}=\mathcal{O}(r^{-2(1-\alpha)})$ (either kinematic regime) give
    $\Theta(\det\bm{F})=\mathcal{O}\big((\log\tfrac1r)^p\big)$, and
...
% The kinematic input for the corner theory: the Williams ansatz as a tensor statement, with its angular structure.
The corner statements of Theorem~\ref{thm:A} require one further input, and it is kinematic: the norm bound of
Lemma~\ref{lem:Abound} controls the \emph{size} of the tangent but not its \emph{limit} at the vertex, and it is the
limit that fixes the corner pencil. We therefore impose the Williams ansatz in tensor form and record its consequence,
which is stronger than one might expect: at the vertex the nonlinear tangent does not converge to the linearised
elasticity tensor at all --- it converges to the \emph{scaled identity} $\mu_e\mathbb{I}$, because every nonlinear term
of \eqref{eq:Lnh} carries two factors of $\bm{F}^{-T}$ and the extreme stretch drives $\bm{F}^{-1}\to0$. The large
deformation \emph{screens} the volumetric coupling.

\begin{assumption}[Williams-ansatz kinematics, (W)]
    \label{ass:williams}
    On $B_\rho$ the state satisfies
    $\bar{\bm{F}}(r,\theta)=\bm{I}+K\,r^{\alpha-1}\big(\bm{G}(\theta)+o(1)\big)$ uniformly in $\theta$, with
    $\bm{G}(\theta)=\alpha\,\bm{\Phi}(\theta)\otimes\hat{\bm{e}}_r+\bm{\Phi}'(\theta)\otimes\hat{\bm{e}}_\theta$ the
    gradient profile of \eqref{eq:kondratiev}. The profile is \emph{nondegenerate}: the angular determinant
    $\mu(\theta):=\det\bm{G}(\theta)=\alpha\,\bm{\Phi}(\theta)\!\times\!\bm{\Phi}'(\theta)$ is nonnegative on
    $[0,\pi/2]$ with finitely many zeros, each of finite order, and $\tr\bm{G}\neq0$ at each zero. (The Dirichlet
    condition $\bm{\Phi}(0)=\bm{0}$ forces a zero of order two at the clamped face; nonnegativity of $\mu$ is the
    orientation consistency $J>0$ of the ansatz itself.)
\end{assumption}

\begin{lemma}[Vertex screening of the tangent]
    \label{lem:screen}
    Let Assumptions~\ref{ass:reg}, \ref{ass:growth}, \ref{ass:mild} and (W) hold, set
    $r_K:=|K|^{-1/(1-\alpha)}$, and define the \emph{degenerate-ray layers}
    \begin{equation}
        \label{eq:layers}
        \Lambda(r):=\Big\{\theta\in[0,\tfrac\pi2] : \mu(\theta)\le 2\,\|\bm{G}\|_\infty\,(r/r_K)^{1-\alpha}\Big\},
        \qquad |\Lambda(r)|\le C\,(r/r_K)^{(1-\alpha)/q_0},
    \end{equation}
    with $q_0$ the maximal order of the zeros of $\mu$. Then, for $r$ small:
    \begin{enumerate}
        \item[\textup{(i)}] \emph{Off the layers} ($\theta\notin\Lambda(r)$) both transverse stretches diverge,
              $J= K^2r^{2(\alpha-1)}\big(\mu(\theta)+o(1)\big)$, the inverse gradient decays algebraically,
              $\|\bar{\bm{F}}^{-1}\|\le C\,(r/r_K)^{1-\alpha}/\mu(\theta)$, and hence
              \begin{equation}
                  \label{eq:screen}
                  \big\|\mathbb{L}(\bar{\bm{F}})-\mu_e\mathbb{I}\big\|
                  \;\le\; \Theta(J)\,\|\bar{\bm{F}}^{-1}\|^2
                  \;\le\; C\,\big(\log\tfrac1r\big)^{p}\,(r/r_K)^{2(1-\alpha)}\,\mu(\theta)^{-2}\;\longrightarrow\;0 .
              \end{equation}
        \item[\textup{(ii)}] \emph{On the layers} the one-large-stretch regime of \S\ref{sub:growth} holds:
              $J\sim K r^{\alpha-1}\tr\bm{G}$, the smallest stretch is bounded below by a constant $c(\bm{G})>0$ ---
              so (NC) holds there automatically --- and $\|\mathbb{L}\|\le\mu_e+C(\log\tfrac1r)^p$ by
              Lemma~\ref{lem:Abound}.
        \item[\textup{(iii)}] Consequently $\mathbb{L}(\bar{\bm{F}}(r,\cdot))\to\mu_e\mathbb{I}$ in $L^q(0,\pi/2)$ for
              every $q<\infty$ at an algebraic rate, though not in $L^\infty$; and off the layers the
              Legendre--Hadamard form $\mu_e|\bm m|^2|\bm N|^2+f''(J)J^2\xi^2$ of \eqref{eq:LHnh} satisfies
              $|f''(J)J^2|\,\|\bar{\bm{F}}^{-1}\|^2\to0$, so strong ellipticity holds down to the vertex there
              \emph{even where} $f''<0$.
    \end{enumerate}
\end{lemma}

\begin{proof}
    In two dimensions the cofactor is affine, $\operatorname{cof}(\bm{I}+\bm{M})=\bm{I}+\operatorname{cof}\bm{M}$, and
    $\det(\bm{I}+\bm{M})=1+\tr\bm{M}+\det\bm{M}$. With $\bm{M}=K r^{\alpha-1}(\bm{G}+o(1))$ the determinant reads
    $J=1+Kr^{\alpha-1}\big(\tr\bm{G}+o(1)\big)+K^2r^{2(\alpha-1)}\big(\mu(\theta)+o(1)\big)$. Off the layers the
    quadratic term dominates by the defining inequality of \eqref{eq:layers}, giving the stated $J$ and, from
    $\bar{\bm{F}}^{-1}=\operatorname{cof}\bar{\bm{F}}^{\,T}/J$ with
    $\|\operatorname{cof}\bar{\bm{F}}\|\le 1+|K|r^{\alpha-1}\|\bm{G}\|_\infty$, the decay of
    $\|\bar{\bm{F}}^{-1}\|$. The bound \eqref{eq:screen} then follows from \eqref{eq:Lnh}: both nonlinear terms are
    quadratic contractions of $\bm{F}^{-T}$ with scalar prefactors dominated by $\Theta(J)$
    (proof of Lemma~\ref{lem:nh}), and $\Theta(J)=\mathcal{O}((\log\tfrac1r)^p)$ by Assumption~\ref{ass:mild}. On a
    layer, $\mu$ is small, the linear term $Kr^{\alpha-1}\tr\bm{G}$ dominates $J$ (using $\tr\bm{G}\neq0$ at the zeros
    of $\mu$), $\bm{M}$ is a bounded perturbation of a large near-rank-one matrix, and
    $\lambda_{\min}=J/\lambda_{\max}\to \tr\bm{G}/\|\bm{G}\|$-type constants: (NC) holds with a computable $c$. The
    measure bound in \eqref{eq:layers} is the finite-order condition on the zeros of $\mu$. Part (iii) integrates
    \eqref{eq:screen} in $\theta$ against the layer bound, and the ellipticity statement is \eqref{eq:LHnh} with
    $|\xi|\le|\bm m||\bm N|\,\|\bar{\bm F}^{-1}\|$. \qed
\end{proof}

% What the screening lemma does to the corner picture: pencil at the vertex is the Laplacian, elasticity is intermediate.
Lemma~\ref{lem:screen} replaces the naive expectation that the corner pencil of the nonlinear tangent should be that of
linearised elasticity. It cannot be: for any $K\neq0$ the state leaves every neighbourhood of $\bm{I}$ as $r\to0$, so
$\mathbb{L}(\bar{\bm{F}})$ never returns to $\mathbb{L}(\bm{I})$ at the vertex --- smallness of $|K|$ only shrinks the
crossover radius $r_K$ below which the field is large. What the tangent \emph{does} approach is $\mu_e\mathbb{I}$, the
decoupled vector Laplacian, whose mixed Dirichlet--Neumann pencil on the quarter-plane has spectrum
$s\in\{1,3,5,\dots\}$ (each Cartesian component separates, $\sin(s\theta)$ with $\cos(s\pi/2)=0$): \emph{no exponent in
$(0,1)$}. The corner operator is therefore a \emph{two-scale} operator --- linearised elasticity with the Williams
exponents $\alpha(\nu)$ on the intermediate annulus $r_K\ll r\ll\rho$, the Laplacian pencil with Lipschitz-regular
increments at the vertex itself --- and since every inner exponent exceeds $1>\alpha$, the $r^\alpha$ Williams field
remains the dominant asymptotics visible from any fixed scale. This is the precise content we can prove of the slogan
that the tensile corner \emph{passes like linear elasticity}: it passes like linear elasticity on the mesh-relevant
intermediate range, and \emph{better} than linear elasticity at the vertex.

\begin{remark}[Status of (W): verified above $\nu_W\approx0.40$, fails below]
    \label{rem:Wstatus}
    Hypothesis (W) is a parameter-free, closed-form condition on the computed clamped--free profile, and checking it
    (\S\ref{sub:numW}) returns a sharp split of the physical range: $\mu\ge0$ holds precisely for
    $\nu>\nu_W\approx0.404$ (single order-two zero at the clamped face, $\tr\bm G\neq0$ there), while for $\nu<\nu_W$
    the determinant is \emph{negative} on a clamped-side sector $(0,\theta_z(\nu))$ that widens as $\nu$ decreases.
    Since the mode enters $\mu$ quadratically, the sign is intrinsic, and $J\sim K^2r^{2(\alpha-1)}\mu$ at depth for
    either load sign: below $\nu_W$ the linear Williams field is orientation-reversing at depth on an $\mathcal O(1)$
    sector, so the tensor ansatz cannot describe the state below the crossover scale there and the screened-vertex
    conclusion of Theorem~\ref{thm:A}(A) is established only on the complementary free-face sector. Nothing above the
    crossover scale is affected --- the intermediate-annulus asymptotics, which is what the blow-up reduction and the
    numerics consume, holds for all $\nu$ --- but a complete vertex theory for $\nu<\nu_W$ requires the
    self-consistent nonlinear inner state on the negative sector, the same open problem as the compressive inner
    region of Remark~\ref{rem:core}. The layer structure, by contrast, is confirmed across the whole range:
    $\tr\bm G\neq0$ at every zero of $\mu$, so the one-large-stretch regime of \S\ref{sub:growth} is exactly the
    layer regime, as Lemma~\ref{lem:screen} asserts.
\end{remark}
...
\begin{theorem}[Well-posedness of the linearised step, tensile corner]
    ...
    Assumptions~\ref{ass:reg}--\ref{ass:mild}; the class is nonempty with both moduli free ---
    Remark~\ref{rem:tempered} --- the simplest exemplars being the pure logarithmic barrier $f(J)=-\mu_e\log J$ and
    the tempered penalties \eqref{eq:ftempered}).} Then the dilation core is empty,
    $r_*=0$, ellipticity holds on the \emph{full} corner neighbourhood $\Omega=B_\rho$ \emph{including the vertex}, and
    \eqref{eq:Abounded}--\eqref{eq:Agaarding} hold on $V^{1,2}_{\beta_\star}(B_\rho)$. Hence the linearised corner
    operator is Fredholm of index zero in $V^{1,2}_{\beta_\star}(B_\rho)$. Under the kinematic hypothesis (W) the
    vertex asymptotics are \emph{two-scale} (Lemma~\ref{lem:screen}): at the vertex itself the governing pencil is that
    of the screened limit $\mu_e\mathbb{I}$ --- the mixed Dirichlet--Neumann vector-Laplacian pencil, with exponents
    $s\in\{1,3,5,\dots\}$, so the Newton increments are \emph{regular} at the tip --- while the Williams exponents
    $\alpha(\nu)$ of linearised elasticity govern the intermediate annulus $r_K\ll r\ll\rho$ and remain the dominant
    asymptotics visible from any fixed scale, every inner exponent exceeding $1>\alpha$. This is the genuine corner
    result.
...
\begin{proof}
    ...
    comparable there, so the same-space estimate is obtained by perturbation off a constant-coefficient symbol. The
    correct reference symbol at the vertex is \emph{not} $\mathbb{L}_{\mathrm{lin}}=\mathbb{L}(\bm{I})$: for any
    $K\neq0$ the state leaves every neighbourhood of $\bm{I}$ as $r\to0$, so
    $\mathbb{L}-\mathbb{L}_{\mathrm{lin}}$ does not vanish at the vertex, and a non-vanishing (let alone
    logarithmically growing) principal-symbol perturbation shifts a corner pencil --- the class $o(r^{-\delta})$ for
    every $\delta$ contains bounded perturbations and is \emph{not} an admissible perturbation class for pencil
    stability. The correct reference symbol is the screened limit of Lemma~\ref{lem:screen}: write
    $\mathbb{L}=\mu_e\mathbb{I}+\mathbb{R}$, so that under (W) the perturbation obeys
    $\|\mathbb{R}\|\le\varepsilon(r)\to0$ algebraically off the layers $\Lambda(r)$ of \eqref{eq:layers}, and
    $\|\mathbb{R}\|=\mathcal{O}((\log\tfrac1r)^p)$ on the layers themselves, whose angular measure vanishes
    algebraically. The off-layer part is a classical vanishing perturbation in the sense of Kondratiev--Maz'ya
    \cite{KMR2001}, under which the admissible weight interval of the reference (vector-Laplacian) pencil is unmoved
    and the \emph{full} pair of estimates \eqref{eq:Abounded}--\eqref{eq:Agaarding} transfers to $\mathbb{L}$ on
    $V^{1,2}_{\beta_\star}(B_\rho)$; the layer part is controlled in the dual pairing by the $\delta$-slack exactly as
    above, and its absorption into the pencil statement is the perturbation step singled out at the end of the proof.
    ...
    $\beta_\star$ avoids the spectrum of the governing corner pencil. In case (A), where $\Omega=B_\rho$ contains the
    vertex, the governing pencil is fixed by the vertex limit of the coefficients, and by Lemma~\ref{lem:screen} that
    limit is $\mu_e\mathbb{I}$: the vertex pencil is the mixed Dirichlet--Neumann pencil of the decoupled vector
    Laplacian, with spectrum $\{1,3,5,\dots\}$ and hence no eigenvalue in the strip $0<\Re\lambda<1$; the energy weight
    is admissible with room to spare, and the elastic expansion \eqref{eq:kondratiev} with the Williams exponents
    $\alpha$ is recovered as \emph{intermediate} asymptotics on $r_K\ll r\ll\rho$, where $\bar{\bm F}\approx\bm I$ and
    $\mathbb{L}\approx\mathbb{L}_{\mathrm{lin}}$. Two technical steps in this transfer are stated as such rather than
    hidden: the absorption of the degenerate-ray layers (a perturbation bounded, up to a logarithm, on angular sets of
    algebraically vanishing measure, to be handled by higher integrability of the gradient on dyadic annuli), and the
    transmission of the Dirichlet condition through the collapsing elastic boundary layer at the clamped face, inside
    which the field stays moderate. Both are perturbation statements about a \emph{vanishing} deviation from the
    screened limit --- an entirely lesser order of difficulty than the pencil statement for the non-vanishing deviation
    from $\mathbb{L}_{\mathrm{lin}}$ that a naive comparison with linear elasticity would require. In case (B), where
    $\Omega=A_*$ omits the vertex, the same estimates give Fredholmness and
...
\begin{remark}[The dilation core]
    ...
    core $\{r\le r_*\}$, where $J>J_*$ and $f_{\log}$ is not strongly elliptic. Four readings are consistent with the
    ...
    ($f''\ge0$, $r_*=0$) removes the issue entirely and recovers the unrestricted statement. (iv) \emph{Screened core
    geometry}: under the kinematic hypothesis (W) the core is not, in fact, a full disc. By Lemma~\ref{lem:screen}(iii)
    the Legendre--Hadamard defect $f''(J)J^2\xi^2$ is damped by $\|\bar{\bm F}^{-1}\|^2\to0$ off the degenerate-ray
    layers, so strong ellipticity survives \emph{down to the vertex} there even though $f''<0$; the genuinely delicate
    set shrinks to the layers $\Lambda(r)$ of \eqref{eq:layers} together with a bounded transition collar $r\sim r_K$
    where the field is $\mathcal{O}(1)$-nonlinear. One caution on the finite-extensibility reading sometimes offered
    here: a volumetric law that caps the dilation, $J<J_{\max}$, is \emph{incompatible} with the assumed corner
    kinematics rather than a cure for it --- under (NC) and $\lambda_{\max}\sim|K|r^{\alpha-1}$ one has
    $J\ge c\,\lambda_{\max}\to\infty$, so a capped material cannot carry the Williams blow-up at all and the state
    must reorganise, changing the ansatz rather than the tangent. The legitimate realisation of the
    finite-extensibility intuition is the one-sided tempered penalty of Remark~\ref{rem:tempered}(iii): a $C^3$
    energy, identical to $f_{\log}$ throughout compression, whose softened dilation tail keeps $f''>0$ globally and
    so removes the core ($r_*=0$) while leaving both moduli free. The dilation core is thus a feature of the specific
    large-$J$ tail of $f_{\log}$, in a regime never physically constrained, not a physical catastrophe.
\end{remark}
...
\begin{remark}[Self-consistency of (NC)]
    ...
    $\lambda_{\max}\to\infty$ while $\lambda_{\min}=\det\bm{F}/\lambda_{\max}$ need not approach zero. The angular
    structure is made quantitative by Lemma~\ref{lem:screen} and checked in \S\ref{sub:numW}: off the degenerate-ray
    layers both stretches diverge (so (NC) holds with room to spare), on the layers $\lambda_{\min}$ stays
    $\mathcal{O}(1)$, and the orientation clause of (W) holds for $\nu>\nu_W\approx0.40$. The complementary
...
\S\ref{sub:growth}),\claude{Same kinematic caveat as (W) on the tensile side: at depth the Williams field has
$J\sim K^2r^{2(\alpha-1)}\mu(\theta)>0$ for $\nu>\nu_W$ regardless of load sign, so the compressive collapse
$J\to0$ lives in the transition annulus $r\sim r_K$, not at the vertex --- consistent with the tip-disc picture of
Prop.~\ref{prop:tipdisc}, but worth one sentence here.}
\begin{equation}
...
$\epsilon=0$ that continues the low-load compressive solution --- constructed in Lemma~\ref{lem:fundexist} and
regulated by Assumption~\ref{ass:fp} below. We use the barrier penalty $f_{\log}$ throughout, so the
...
% The fundamental path must be constructed, not defined by fiat: existence is variational and provable in 2D for a
% convex penalty; what cannot be proved is collected in one standing assumption, stated before it is first consumed.
The fundamental path deserves an existence statement rather than a definition by fiat. Existence is variational and,
in two dimensions, genuinely provable --- for a \emph{globally convex} volumetric penalty, which is where the tempered
class of Remark~\ref{rem:tempered} pays a second dividend; what resists proof is the \emph{regularity} of the path,
and we collect exactly the regularity that the analysis below consumes into one standing assumption, stated here,
before its first use, rather than discovered midway.

\begin{lemma}[Existence and energy bound for the fundamental state]
    \label{lem:fundexist}
    Let $d=2$, let the loading be displacement-controlled ($\ell_t$ realised as boundary data $t\,\bm g$ on the
    Dirichlet part, $\epsilon=0$), and let the volumetric penalty $f$ be convex on $(0,\infty)$ with the barrier
    $f(J)\to\infty$ as $J\to0^+$ (the class of Remark~\ref{rem:tempered}; in particular the one-sided tempered
    penalty, which coincides with $f_{\log}$ throughout compression). Extend $f\equiv+\infty$ on $(-\infty,0]$. Then
    for every $t$ the energy $\mathcal E_t(\bm u)=\int_{B_\rho}\Psi(\bm I+\Grad\bm u)\,dV$ attains a global minimiser
    $\bm u_0(t)$ over the admissible class $\{\bm u\in H^1:\ \bm u=t\bm g \text{ on } \Gamma_D\cup\text{outer arc}\}$,
    and every minimiser satisfies $J_0>0$ a.e. The minimum value $E_{\min}(t)$ is continuous in $t$ and obeys
    $E_{\min}(t)\le C\,t^2$ for any smooth extension competitor that stays orientation-preserving on $[0,1]$.
\end{lemma}

\begin{proof}
    Coercivity: $\Psi\ge\tfrac{\mu_e}{2}|\bm F|^2+f(J)$ with $f$ convex, bounded below by an affine function of $J$
    and $|J|\le\tfrac12|\bm F|^2$, so minimising sequences are bounded in $H^1$; pass to a weak limit. In $d=2$ the
    cofactor is \emph{affine} in $\bm F$, whence for $\bm u_k\rightharpoonup\bm u$ in $H^1$ the identity
    $\int J_k\,\varphi=-\tfrac12\int \bm u_k\cdot(\operatorname{cof}\Grad\bm u_k)\Grad\varphi+\dots$ pairs a strongly
    $L^2$-convergent factor (Rellich) with a weakly convergent one: $\det\Grad\bm u_k\to\det\Grad\bm u$ in
    $\mathcal D'$, and since the $J_k$ are bounded in $L^1$ with a distributional limit that is itself the $L^1$
    function $J$, no singular part can form. Lower semicontinuity: the quadratic term is convex; for the penalty,
    $f=\sup_\ell \ell$ over its affine minorants, $\int \ell(J_k)\to\int \ell(J)$ for each $\ell$, and monotone
    approximation gives $\liminf\int f(J_k)\ge\int f(J)$ (Goffman--Serrin; the barrier side is Fatou). Finite energy
    forces $J_0>0$ a.e. The bound $E_{\min}(t)\le\mathcal E_t(t\bm G)\le Ct^2$ is Taylor expansion of $\Psi$ at
    $\bm I$ along the smooth competitor; continuity of $E_{\min}$ follows by comparing minimisers at nearby loads
    after correcting the boundary data with the same extension and using the lower semicontinuity just proved. \qed
\end{proof}

% What the lemma does not give, stated honestly, and bundled into the one standing assumption on the path.
Three caveats calibrate the lemma. For $f_{\log}$ itself the direct method is obstructed by the same large-$J$
nonconvexity that creates the dilation core of Lemma~\ref{lem:Aellip} --- one more reason to temper the tail
(Remark~\ref{rem:tempered}); minimisers need not be unique, so a \emph{selection} $t\mapsto\bm u_0(t)$ continuous in
$t$ is not automatic; and minimisers of polyconvex energies carry essentially no regularity theory, so nothing in the
lemma bounds $\lambda_{\min}(\bm F_0)$ below or gives the state a corner expansion. Those are precisely the
unprovable inputs, and we now name them. Define the pointwise-collapse load
$t_{\mathrm{coll}}:=\inf\{t:\ \operatorname*{ess\,inf}_{B_\rho}\lambda_{\min}(\bm F_0(t))=0\}$.

\begin{assumption}[Regular fundamental path, (FP)]
    \label{ass:fp}
    There is a selection $t\mapsto\bm u_0(t)$ of solutions of \eqref{eq:Gdef} with $\epsilon=0$, continuous into
    $H^1$ with $\bm u_0(0)=\bm 0$ (for a convex penalty, a selection of the minimisers of
    Lemma~\ref{lem:fundexist}), such that:
    \begin{enumerate}
        \item[\textup{(i)}] \emph{Pre-collapse non-degeneracy.} $t_{\mathrm{coll}}>0$, and for every $\bar t<t_{\mathrm{coll}}$
              there is $c(\bar t)>0$ with $\lambda_{\min}(\bm F_0(t))\ge c(\bar t)$ a.e.\ on $B_\rho$ for all
              $t\le\bar t$; consequently $t\mapsto\mathbb{L}(\bm F_0(t))$ is bounded and continuous into $L^\infty$ on
              $[0,\bar t]$.
        \item[\textup{(ii)}] \emph{Corner expansion.} For $t<t_{\mathrm{coll}}$ the state admits the expansion
              $\bm u_0=K(t)\,r^\alpha\bm\Phi(\theta)+\bm u_0^\sharp$ with the pointwise gradient envelope
              $|\Grad\bm u_0^\sharp|\le C_\sharp(t)(r/\rho)^{\alpha'-1}$, $\alpha'>\alpha$, and $C_\sharp$, $K$
              bounded on compact subsets of $[0,t_{\mathrm{coll}})$.
        \item[\textup{(iii)}] \emph{Monotone load-to-stretch map.} Condition \textup{(M)}: at each material point the
              relevant stretches $t\mapsto\lambda_\parallel$, $\lambda_{\min}$ are nonincreasing along the path.
    \end{enumerate}
\end{assumption}

\begin{remark}[What (FP) costs, and how it removes a circularity]
    \label{rem:fp}
    Part (i) is the compressive analogue of (NC) and cannot be discharged by the tensile screening of
    Lemma~\ref{lem:screen}: under compression the inverse channel $J\to0$ has no analogous cancellation, so the
    coefficient field of the state genuinely may degenerate, and (i) is an assumption about where --- namely, not
    before $t_{\mathrm{coll}}$, which (i) merely asserts is positive. Part (ii) is likewise a hypothesis, not a
    citation: the linear Kondratiev theory applies to the \emph{nonlinear} state only through a
    frozen-coefficient reading whose vertex behaviour is exactly the open inner problem of Remark~\ref{rem:core}.
    The gain from stating (FP) here is logical hygiene downstream: previously, continuity of the second variation
    for $t<t_c$ silently consumed a $\lambda_{\min}$ floor that only the later buckling-precedes-collapse condition
    (BC) supplies --- a circle. Under (FP) the circle opens into a line: Lemma~\ref{lem:tc} needs (i) only on
    $[0,t_*]$, where $t_*$ is the load at which a destabilising field first exists; if $t_*<t_{\mathrm{coll}}$
    --- the single quantitative input, a tip floor at the \emph{Biot} load, not at collapse --- then the first
    crossing satisfies $t_c\le t_*<t_{\mathrm{coll}}$, and the ordering (BC) is a \emph{conclusion}. The residual
    open content is thus one lower bound on $t_{\mathrm{coll}}$, quantified in the concrete setting by the tip-disc
    estimate there, and nothing else.
\end{remark}
...
\begin{lemma}[Existence of the critical load, and the ordering it inherits]
    \label{lem:tc}
    Assume (i) coercivity at zero load, $\mu_1(0)>0$; (ii) a structural instability of the compressed state: there
    exist $t_*\in(0,1)$ and $\bm{v}_*\in V$ with $Q_{t_*}(\bm{v}_*)<0$; and (iii) the destabilisation precedes
    collapse, $t_*<t_{\mathrm{coll}}$, with (FP)(i) in force. Then $\mu_1$ is finite and continuous on $[0,t_*]$,
    there is a first $t_c\in(0,t_*]$ with $\mu_1(t_c)=0$ and $\mu_1(t)>0$ for $t<t_c$, and the buckling load
    automatically satisfies the ordering $t_c\le t_*<t_{\mathrm{coll}}$.
\end{lemma}
\begin{proof}
    At $t=0$, $\bm{F}_0=\bm{I}$ and $\mathbb{L}=\mu_e\mathbb{I}+(\text{volumetric at }J{=}1)$ is positive definite, so
    with Korn's inequality $\mu_1(0)>0$. Hypothesis (ii) gives $\mu_1(t_*)\le Q_{t_*}(\bm{v}_*)/\|\bm{v}_*\|_V^2<0$.
    By (iii) and (FP)(i) the coefficients $\mathbb{L}(\bm{F}_0(t))$ are bounded in $L^\infty$ \emph{uniformly} on
    $[0,t_*]$ and continuous in $t$ --- this is where the finiteness of $Q_t$ comes from, and it is not free: without
    the floor $\lambda_{\min}\ge c(t_*)$ the form need not even be bounded --- so $\mu_1$ is continuous, and the
    intermediate value theorem gives the first zero $t_c\le t_*<t_{\mathrm{coll}}$. \qed
\end{proof}
...
corner as essential spectrum) and \emph{simplicity}. Simplicity is generic --- in the precise sense of holding for
almost every point of the wedge parameter family $(\nu,\hat K)$, the only sense in which \emph{generic} carries
content for a fixed body --- and, for the model bodies, can be argued
...
geometry-dependent orthogonal-mode feedback overturning these signs; we record the cubic clause as a residual
hypothesis to be verified numerically (Remark~\ref{rem:bsign}) rather than asserted from the creasing analogy
\cite{Biot1963,Hong2009} --- and the computation finds the feedback does overturn the cubic but \emph{not} the
quintic (Remark~\ref{rem:quinticstatus}), which reshapes the saturation story below.
\begin{assumption}[Subcritical sign pattern (residual, geometry-dependent)]
    \label{ass:subcrit}
    With $c_2=0$ by Lemma~\ref{lem:volcoeff}(i), the full reduced cubic coefficient
    \begin{equation}
        b:=\tfrac16\langle\bm{\phi},\partial^3_{\bm{u}}G[\bm{\phi},\bm{\phi},\bm{\phi}]\rangle,
    \end{equation}
    the frozen part of Lemma~\ref{lem:volcoeff} corrected by the orthogonal-mode feedback, satisfies $b<0$
    (subcritical). This is a condition on the corner geometry, not on the material model. We do \emph{not} assume a
    sign for the reduced quintic $d$ (defined analogously at fifth order): its sixth-order reduction is derived in
    Lemma~\ref{lem:quintic} and \emph{computed} in \S\ref{sub:numb}, with the result $d<0$ --- the polynomial normal
    form does not saturate the subcritical branch, and the arrest of the snap is a finite-amplitude matter carried by
    Assumption~\ref{ass:nd} (Remark~\ref{rem:quinticstatus}).
\end{assumption}
\begin{remark}[Status of the subcritical signs: geometry-decided, checked numerically]
    ...
    $|b_\psi|>b_{\mathrm{dir}}$; the quintic is treated at sixth order in Lemma~\ref{lem:quintic}, with the outcome
    $d<0$ (Remark~\ref{rem:quinticstatus}). The magnitude is evaluated
...
\begin{proposition}[Normal form]
    ...
    \end{equation}
    with $a>0$ (Lemma~\ref{lem:transversal}), $b<0$ (Assumption~\ref{ass:subcrit}), $d$ the reduced quintic of
    Lemma~\ref{lem:quintic} (computed negative; no statement below uses a sign for it except where explicitly
    conditioned), and $\eta$ as in Lemma~\ref{lem:eta}.
\end{proposition}
...
\begin{proof}
    ...
    Lemma~\ref{lem:transversal}, the cubic $bw^3$ from Assumption~\ref{ass:subcrit} and the quintic $dw^5$ from
    Lemma~\ref{lem:quintic} (the quadratic
    vanishing by symmetry), and the inhomogeneity $\epsilon\eta$ from $-\epsilon\langle\bm{p},\bm{\phi}\rangle$. Because
    $b<0$, the cubic alone does not saturate the amplitude, which is why the quintic $dw^5$ is retained --- whether it
    saturates in turn is decided by the sign of $d$ (Remark~\ref{rem:quinticstatus}). \qed
\end{proof}
...
\begin{remark}[What remains open in Assumption~\ref{ass:subcrit}]
    ...
    $b$ is a datum of the critical point). Second, the same argument does \emph{not} sign the \emph{full} quintic
    feedback --- the sixth-order reduction mixes terms of both signs --- but it does sign one channel of it, and the
    computation built on the next lemma settles the rest: see Lemma~\ref{lem:quintic} and
    Remark~\ref{rem:quinticstatus}.
\end{remark}

% The sixth-order reduction, never previously written down: it makes d computable, signs the psi3 channel, and the
% computation then returns d<0 -- the saturation clause of the old normal form is refuted, not merely open.
\begin{lemma}[The sixth-order reduction, and the quintic feedback channel that is never stabilising]
    \label{lem:quintic}
    In the setting of Lemma~\ref{lem:fbsign} (symmetric case, $c_2=0$), let $\bm\psi_2=-(Q\mathcal{L}_cQ)^{-1}R$ and
    define $S_3\in(V^\perp)'$ by
    \begin{equation}
        \label{eq:S3def}
        \langle S_3,\bm v\rangle := E_3[\bm\phi,\bm\psi_2,\bm v] + \tfrac16\,E_4[\bm\phi,\bm\phi,\bm\phi,\bm v],
        \qquad \bm v\in V^\perp ,
    \end{equation}
    so that the third-order Lyapunov--Schmidt field is $\bm\psi_3=-(Q\mathcal{L}_cQ)^{-1}S_3$. Then the reduced
    quintic coefficient of \eqref{eq:bifeq} is exactly
    \begin{equation}
        \label{eq:dformula}
        d \;=\; \tfrac1{120}\,E_6[\bm\phi^{\otimes6}]
        \;+\; \tfrac14\,E_5[\bm\phi^{\otimes4},\bm\psi_2]
        \;+\; \tfrac32\,E_4[\bm\phi^{\otimes2},\bm\psi_2^{\otimes2}]
        \;+\; E_3[\bm\psi_2^{\otimes3}]
        \;-\; 3\,\big\langle S_3,\ (Q\mathcal{L}_cQ)^{-1}S_3\big\rangle .
    \end{equation}
    The last term is $\le0$, with equality iff $S_3=0$: the feedback through $\bm\psi_3$ is \emph{never stabilising}
    for the quintic, the exact mirror of Lemma~\ref{lem:fbsign} one order up. In the frozen limit
    $D_1\to0$ the first term reduces to $d_{\mathrm{dir}}=\int f'''(J_0)\,d_\phi^3\,dV$ of
    Lemma~\ref{lem:volcoeff}(ii); the middle three terms are unsigned in general.
\end{lemma}

\begin{proof}
    Write $\bm u=\bm u_0+w\bm\phi+\bm\psi(w)$ with $\bm\psi=w^2\bm\psi_2+w^3\bm\psi_3+\mathcal O(w^4)$ and expand the
    reduced energy $V(w)=\mathcal E_{t_c}(\bm u)$ at $\epsilon=0$. Because $V$ is stationary in $\bm\psi$ on the range
    component, perturbing $\bm\psi$ by $\mathcal O(w^4)$ changes $V$ by $\mathcal O(w^8)$, so the fourth-order field
    does not enter the $w^6$ coefficient. Collecting the order-$w^6$ terms of $\sum_k\tfrac1{k!}E_k[(w\bm\phi+
    w^2\bm\psi_2+w^3\bm\psi_3)^{\otimes k}]$ gives
    \[
        V_6=\tfrac1{720}E_6[\bm\phi^{\otimes6}]+\tfrac1{24}E_5[\bm\phi^{\otimes4},\bm\psi_2]
        +\tfrac14E_4[\bm\phi^{\otimes2},\bm\psi_2^{\otimes2}]+\tfrac16E_3[\bm\psi_2^{\otimes3}]
        +\langle S_3,\bm\psi_3\rangle+\tfrac12\langle\bm\psi_3,\mathcal{L}_c\bm\psi_3\rangle ,
    \]
    where the $w^3$ range equation $Q(\mathcal{L}_c\bm\psi_3+S_3)=0$ identifies $\bm\psi_3$ and collapses the last
    two terms to $-\tfrac12\langle S_3,(Q\mathcal{L}_cQ)^{-1}S_3\rangle$; with $d=6V_6$ this is \eqref{eq:dformula}.
    Nonpositivity of the final term is positivity of $(Q\mathcal{L}_cQ)^{-1}$ on $V^\perp$ (Lemma~\ref{lem:fbsign}(i)).
    The frozen limit follows from $E_6[\bm\phi^{\otimes6}]=\int f^{(6)}D_1^6+30f^{(5)}D_1^4d_\phi
    +180f''''D_1^2d_\phi^2+120f'''d_\phi^3\,dV$ (Fa\`a di Bruno with the exact quadratic determinant expansion of
    Lemma~\ref{lem:relief}) at $D_1\to0$. \qed
\end{proof}

\begin{remark}[Status of the quintic: computed, and negative]
    \label{rem:quinticstatus}
    Formula \eqref{eq:dformula} makes $d$ computable with the same ingredients as $b$ --- one further bordered solve
    for $\bm\psi_3$ and closed-form volumetric integrands (in two dimensions the determinant is quadratic in the
    gradient, so every $E_k$ with $k\ge3$ is an explicit polynomial in $D_1$-type contractions). The computation
    (\S\ref{sub:numb}) returns $d<0$ at \emph{every} tested point of the $(\nu,\hat K)$ rectangle, under both grading
    profiles and at every resolution, with the sign-definite $\bm\psi_3$-channel of \eqref{eq:dformula} as the
    dominant contribution. The saturation clause $d>0$ of the classical subcritical normal form is therefore not
    merely unsupported --- on the accessible discretisation it is \emph{refuted}: with $b<0$ \emph{and} $d<0$ the
    polynomial normal form has no stabilising term through fifth order, the quintic fold \eqref{eq:fold} does not
    exist at this order, and the unstable branch exits the normal-form neighbourhood without turning. This is not a
    pathology of the corner but the expected signature of creasing: the crease is a strongly nonlinear,
    finite-amplitude state not reachable by a weakly nonlinear expansion about the flat (here: compressed) state
    \cite{Hong2009}, and the corner inherits exactly that character. The consequences are catalogued where they
    arise: the snap-load law \eqref{eq:koiter} and the unstable-branch scaling \eqref{eq:wscale} need only $b<0$ and
    are untouched; the arrest of the snap at finite amplitude is carried entirely by the non-degeneracy hypothesis
    (ND) of Assumption~\ref{ass:nd}, which the quintic was never a substitute for; and the fold data $(t_L,w_L)$ of
    \eqref{eq:fold} are meaningful only conditionally on $d>0$, i.e.\ not for the computed corner.
\end{remark}
...
\end{equation}
Were the quintic stabilising, $d>0$, this would admit real nontrivial branches only for $t\ge t_L$, where
\begin{equation}
    \label{eq:fold}
    t_L := t_c - \frac{b^2}{4ad} < t_c, \qquad w_L^2 = \frac{-b}{2d} = \mathcal{O}(1),
\end{equation}
with the two branches meeting in a limit point (fold) at the finite amplitude $w_L$, and the upper branch stable ---
the textbook subcritical picture, in which the normal form itself supplies the arrested state. The computed corner is
\emph{not} in this case: $d<0$ (Remark~\ref{rem:quinticstatus}), the truncation carries no stabilising term, and the
nontrivial branch simply leaves the normal-form neighbourhood without turning --- exactly the behaviour of a crease,
whose arrested state is a finite-amplitude fold invisible to any weakly nonlinear expansion \cite{Hong2009}. What
survives, and what the rest of this section uses, is sign-robust: the fundamental branch $w=0$ is stable for $t<t_c$
and unstable for $t>t_c$; the departure is subcritical (the cubic pushes away from the fundamental branch); the
arrested creased state exists on physical grounds and its bounded distortion is exactly the content of
Assumption~\ref{ass:nd}, which now carries \emph{alone} the coexistence-and-hysteresis picture: on an interval below
$t_c$ the compressed state and the finite-amplitude crease coexist, with a \emph{snap} at $t_c$; only the
\emph{location} of the fold $(t_L,w_L)$ is beyond the reduced equation when $d<0$.
...
\begin{lemma}[Determinant relief]
    ...
    $d_\phi\ge c_d>0$ a.e.\ \emph{on a neighbourhood of the collapse locus} $\Sigma_{\mathrm{c}}$, with a uniform floor
    $c_d$ (Young's inequality in case~(ii) below divides by $d_\phi$, so positivity without a floor would leave the
    constants in \eqref{eq:Jlower} non-uniform on the tube) --- the only region where the
...
\begin{proof}
    ...
    Assumption~\ref{ass:rot}, \emph{together with} the localised orientation $d_\phi\ge c_d>0$ of \eqref{eq:dphisign} --- the one
    ...
    once $J_0$ is small enough that $c_\phi^2 J_0/|\log J_0|\le c_d$ --- a threshold \emph{uniform} on the tube thanks
    to the floor $d_\phi\ge c_d$ --- $\tfrac1{2d_\phi}D_1^2\le\tfrac12 J_0$ and
...
gives only measure-type control of $\{J\ \text{small}\}$, not a pointwise floor. Its role has moreover grown: with
the computed quintic negative (Remark~\ref{rem:quinticstatus}) the polynomial normal form supplies no arrested state,
so (ND) is not a convenience but the \emph{sole} carrier of the post-snap picture. It is the finite-amplitude analogue of
...
\begin{proposition}[Stable physical branch: bounded tangent]
    ...
    compressed branch the bound needs only the pre-collapse floor of (FP)(i) together with the ordering
    $t_c<t_{\mathrm{coll}}$ of Lemma~\ref{lem:tc}: $\lambda_{\min}\ge\lambda_c(t)\ge\lambda_c(t_c)>0$ (buckling
    precedes collapse), so $\|\mathbb{L}\|=\mathcal{O}(1)$ there. Under the non-degeneracy hypothesis~\ref{ass:nd} the post-snap
...
\begin{theorem}[Unstable continuation branch: regularised blow-up]
    \label{thm:reg}
    Under Assumptions~\ref{ass:reg}, \ref{ass:bif}, \ref{ass:subcrit}, \ref{ass:rot}, the orientation floor
    \eqref{eq:dphisign} near the collapse locus, and the conclusions
...
\begin{lemma}[Transversal collapse: $s=1$]
    ...
    be the collapse locus on which the smallest stretch vanishes, $\lambda_{\min}=0$ (equivalently $J=0$). Assume the
    \emph{transversality hypothesis}
    \begin{itemize}
        \item[\textup{(T)}] $\Sigma_{\mathrm{c}}$ is of codimension one, and at each of its points $\lambda_{\min}$ is a
              simple singular value of $\bm{F}$ (hence smooth in $\bm x$) with $\nabla\lambda_{\min}\ne\bm 0$.
    \end{itemize}
    Then $\lambda_{\min}$ vanishes \emph{transversally},
    \begin{equation}
        ...
        \qquad \kappa_*>0 ,
    \end{equation}
    and the localisation exponent is $s=1$. (A tangential contact, $\nabla\lambda_{\min}=\bm 0$ on $\Sigma_{\mathrm{c}}$,
    would give $s\ge2$ and a \emph{wider} tube; Remark~\ref{rem:transgen} states in what sense (T) is generic and what
    survives without it.)
\end{lemma}
...
\begin{proof}
    ...
    hypothesis (T) gives \eqref{eq:transversal} with $\kappa_*=|\nabla\lambda_{\min}|>0$. \qed
\end{proof}

\begin{remark}[In what sense (T) is generic, and what survives without it]
    \label{rem:transgen}
    A word of hygiene on \emph{generic}. For the \emph{fixed} field of a fixed body, material and load path, (T) is
    simply a hypothesis: Sard's theorem asserts that almost every \emph{level} $\delta$ of $\lambda_{\min}$ is regular,
    but $\Sigma_{\mathrm{c}}$ is the specific level $\delta=0$, about which Sard says nothing, and there is no
    parameter being perturbed. Genericity becomes meaningful only relative to a family, and the natural one is already
    at hand: the phase-diagram parameters $(\nu,\hat K)$ of the model wedge together with the load $t$. Within a family
    rich enough to move the field --- versality we do not verify here --- tangential contact of a scalar zero set is a
    Thom-nongeneric event, and this is the honest content of calling (T) generic. Independently, (T) is
    \emph{checkable}: $|\nabla\lambda_{\min}|$ along the computed continuation near its zero set is a by-product of the
    same numerics that locate $\Sigma_{\mathrm{c}}$. Finally, the architecture does not stand or fall with (T). Without
    any transversality, the barrier bound of Lemma~\ref{lem:measrep} confines the sublevel sets unconditionally,
    $|\{J_0\le\delta\}|\lesssim E(t)/\log^2\delta$ --- a \emph{logarithmic} tube in place of the algebraic one --- so
    the qualitative conclusion (the high-stiffness zone is confined and vanishes with $\epsilon$) is hypothesis-free;
    what (T) buys is the sharp algebraic half-width \eqref{eq:locallength} and with it the $q<\tfrac12$ integrability
    range of Proposition~\ref{prop:narrow}. And in the empirically expected geometry of Remark~\ref{rem:locus} --- the
    collapse locus degenerating to the vertex point --- the localisation only improves.
\end{remark}
...
Assumption~\ref{ass:rot}, its qualitative content confirmed numerically, Remark~\ref{rem:rotnum}), the tangent
diverges only algebraically,
...
\input{splitted/concrete_realization.tex}

\input{splitted/numerical.tex}

\section{Conclusions}
\label{sec:conclusions}

\input{splitted/conclusion.tex}
