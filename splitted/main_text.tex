...
\begin{lemma}[Strong ellipticity outside the dilation core]
    ...
    \end{equation}
    On the complementary core $\{r\le r_*\}$, where $J>J_*$, strong ellipticity of $f_{\log}$ is genuinely lost
    (Remark~\ref{rem:SEcaveat}): the bound \eqref{eq:LHcorner} fails and the modulus $\mu_e+f''(J)J^2\,\xi^2/(|\bm m|^2|\bm N|^2)$
    can turn negative.
\end{lemma}
...
$f_{\log}$ itself --- not the corner --- ceases to be strongly elliptic because the material is dilated past $J_*$. The
...
\begin{theorem}[Well-posedness of the linearised step, tensile corner]
    \label{thm:A}
    Let $f=f_{\log}$ and work on the annular corner neighbourhood $A_*=\{r_*<r<\rho\}$ outside the dilation core
    \eqref{eq:rstar} of Lemma~\ref{lem:Aellip} (for a globally convex-volumetric barrier the core is empty, $r_*=0$, and
    $A_*=B_\rho$). Under Assumptions~\ref{ass:reg}--\ref{ass:mild} and the non-compressive hypothesis (NC), the form
    \eqref{eq:formA}, restricted to $A_*$, is bounded and coercive on $V^{1,2}_{\beta_\star}(A_*)$ modulo a compact
    perturbation: there exist constants $\Lambda<\infty$, $\gamma_1>0$ and a compact form $k(\cdot,\cdot)$ such that, for
    all $\bm{u},\bm{v}\in V^{1,2}_{\beta_\star}(A_*)$,
    \begin{align}
        \label{eq:Abounded}
    ...
    \end{align}
    Consequently, on $A_*$, the linearised corner operator is Fredholm of index zero in $V^{1,2}_{\beta_\star}$; for
    $\beta_\star$ avoiding the corner spectrum it is an isomorphism, its solutions admit the asymptotic expansion
    \eqref{eq:kondratiev}, and the governing singular exponents coincide with those of the linearised elasticity
    problem. The inner core $\{r\le r_*\}$, where $f_{\log}$ loses strong ellipticity (Lemma~\ref{lem:Aellip}), is a
    fixed-size region carrying no vertex; it must be treated separately and is the price of the logarithmic penalty in
    extreme tension (see Remark~\ref{rem:core}).
\end{theorem}

\begin{proof}
    \emph{Boundedness.} By Cauchy--Schwarz and Lemma~\ref{lem:Abound}, on $A_*$ the coefficient obeys the sharp bound
    ...
    \begin{align*}
        |a(\bm{u},\bm{v})|
         & \le \int_{A_*} \|\mathbb{L}\|\,|\Grad\bm{u}|\,|\Grad\bm{v}|\,dV
        \le C_\delta \Big(\!\int_{A_*} r^{2\beta_\star}|\Grad\bm{u}|^2\Big)^{1/2}
        \Big(\!\int_{A_*} r^{-2\delta-2\beta_\star}|\Grad\bm{v}|^2\Big)^{1/2}.
    \end{align*}
    This exhibits $a$ as bounded from $V^{1,2}_{\beta_\star}\times V^{1,2}_{\beta_\star-\delta}$. The logarithm is
    ...
    avoids the discrete corner spectrum, hence sits at a positive distance $\delta_0$ from it). Choosing
    $\delta<\delta_0$, the weight $\beta_\star-\delta$ lies in the same admissible interval as $\beta_\star$, the two
    Kondratiev norms $V^{1,2}_{\beta_\star}$ and $V^{1,2}_{\beta_\star-\delta}$ are comparable on $A_*$ (the bounded
    annulus excludes the vertex, where alone they would differ), and \eqref{eq:Abounded} follows with
    $\Lambda=\Lambda(\delta)$. The point is that the logarithmic growth does not move the admissible weight interval; no
    singular weight is ``absorbed'' into $\bm{v}$.

    \emph{G\aa rding inequality.} By Lemma~\ref{lem:Aellip} the symbol $\mathbb{L}(\bm{x})$ is strongly elliptic on
    $A_*$ with the constant Legendre--Hadamard modulus $\mu_e$. Freezing the (measurable, uniformly elliptic)
    coefficients on a dyadic partition $\{r\sim 2^{-j}\}\cap A_*$ and applying the constant-coefficient G\aa rding
    ...
    \[
        a(\bm{v},\bm{v}) \ge \mu_e\,c_K^{-1}\!\int_{A_*} r^{2\beta_\star}|\Grad\bm{v}|^2\,dV
        - C\!\int_{A_*} r^{2\beta_\star}|\bm{v}|^2\,dV,
    \]
    ...
    Kondratiev--Maz'ya framework \cite{KMR2001}: it is Fredholm of index zero in $V^{1,2}_{\beta_\star}(A_*)$ whenever
    $\beta_\star$ avoids the spectrum of the corner operator pencil. Because the nonlinear correction to the principal
    symbol is subordinate (it is $o(r^{-\delta})$ for every $\delta$, by Lemma~\ref{lem:Abound}), the pencil
...
\begin{remark}[The dilation core]
    ...
    ($f''\ge0$, $r_*=0$) removes the issue entirely and recovers the unrestricted statement.
\end{remark}
...
\begin{assumption}[Subcritical, non-degenerate reduced potential]
    ...
    $c_2:=\tfrac12\langle\bm{\phi},\partial^2_{\bm{u}}G[\bm{\phi},\bm{\phi}]\rangle$ vanishes whenever the critical mode
    is \emph{antisymmetric} under a reflection symmetry of the loaded configuration that maps each boundary face to
    itself. We stress that this is \emph{not} the corner-bisector reflection $\theta\mapsto\tfrac\pi2-\theta$, which
    interchanges $\Gamma_D$ and $\Gamma_N$ and is therefore not a symmetry of the mixed problem at all; it is the
    lateral reflection $R$ fixing each face, realised concretely for the model bodies in \S\ref{sub:symmetry}. When no
    such symmetry is available, or the ground mode lies in the symmetric sector, $c_2\neq0$ and the reduction is
    transcritical rather than a pitchfork; this case leaves the conclusions of the section intact and is treated in
    Remark~\ref{rem:transcritical}.
\end{assumption}
...
it furnished by the energy barrier (Lemma~\ref{lem:rot}) and show it is dominated by the second-order relief.
...
\begin{lemma}[Determinant relief]
    ...
    \end{equation}
    By the barrier estimate \eqref{eq:rotrate} of Lemma~\ref{lem:rot}, the critical mode is \emph{asymptotically}
    volume-preserving, $|D_1|\le c_\phi\,J_0/\sqrt{|\log J_0|}$, so $D_1$ does not vanish at finite load but is itself
    small with $J_0$. Let
...
\begin{proof}
    ...
    bound the linear term two ways using $|D_1|\le c_\phi J_0/\sqrt{|\log J_0|}$. By Young's inequality
    $|w\,D_1|\le \tfrac12 d_\phi w^2 + \tfrac1{2d_\phi}D_1^2$, and $D_1^2\le c_\phi^2 J_0^2/|\log J_0|\le \tfrac12 d_\phi J_0$
    once $J_0$ is small (the factor $J_0/|\log J_0|\to0$). Hence $|w\,D_1|\le \tfrac12 d_\phi w^2+\tfrac12 J_0$, and
    inserting into \eqref{eq:Jexp} (absorbing the $\mathcal{O}(w^3)$ into the constants for $|w|\le w_0$) yields
    \eqref{eq:Jlower}. Thus the residual first-order term, far from dominating, is swallowed by the barrier-controlled
    $J_0$ and the relief $w^2 d_\phi$ --- this is the precise content of ``buckling relieves compression'' at finite
    load, with no appeal to an exact rotation condition. Since $J=\prod_i\lambda_i$ and the $d-1$ largest stretches are
...
The smallness of the first-order volume change $D_1$ in \eqref{eq:rotcond} is not an extra assumption: it is
\emph{forced}, quantitatively, by the energy barrier, as we now show. This is the precise content of the informal
statement ``buckling relieves compression''---the barrier penalises any finite-energy mode that changes volume at first
order, driving $D_1\to0$ with $J_0$ at the rate \eqref{eq:rotrate}, which (Lemma~\ref{lem:relief}) is fast enough for
the second-order relief to dominate.
