...
\begin{lemma}[Determinant relief]
    ...
    which for $d=2$ reduces to $\det(\Grad\bm{\phi})$. We take the mode to satisfy the relief-orienting genericity
    condition $d_\phi>0$ a.e.\ (a property of the mode, discussed at \eqref{eq:dphisign} in \S\ref{sub:reliefd}; it is
    \emph{not} supplied by the transversality Lemma~\ref{lem:sval}). Then
    \begin{equation}
        \label{eq:Jexp}
